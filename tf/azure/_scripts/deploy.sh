...
if [ $IMAGE_COUNT -gt 0 ]; then
  ...
  UPDATED_IMAGE_COUNT=$(sudo docker ps --filter="name=${APP_ALIAS}*" | grep "${APP_ALIAS}" | wc -l)
  ...
  if [ $UPDATED_IMAGE_COUNT -ge $NEWCOUNT ]; then
    ...
    sudo docker compose up -d --scale app="${2}" --no-recreate
  fi
else
