...
resource "azurerm_application_gateway" "gw_network" {
  ...
  dynamic "backend_address_pool" {
    for_each = local.api_suffixes
    content {
      name                       = "${split(":", backend_address_pool.value)[0]}-routing-tb"
      rule_type                  = "Basic"
      http_listener_name         = "${split(":", backend_address_pool.value)[0]}-listener"
      backend_address_pool_name  = "${split(":", backend_address_pool.value)[0]}-pool"
      backend_http_settings_name = "${split(":", backend_http_settings.value)[0]}-http-setting"
      priority                   = 100
    }
  }
}
